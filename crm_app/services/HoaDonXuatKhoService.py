...
def get_hoa_don_xuat_kho(filter, limit, page, sort, order):
    ...
    get_attr = """
        khach_hang.id AS khach_hang_id, khach_hang.ho_ten AS khach_hang, so_hoa_don, ma_hoa_don,giao_hang.id AS nhan_vien_giao_hang_id, giao_hang.ho_ten AS nv_giao_hang, sale.id AS nv_sale_id, sale.ho_ten AS nv_sale, ngay_xuat, tong_tien, tra_truoc, (tong_tien - tra_truoc) AS con_lai, ghi_chu
    """
    ...
    try:
        ...
        for item in ds_san_pham_xuat:
            result = add_ct_hoa_don_xuat(
                ds_sku=item.get("ds_sku"), hoa_don_id=hoa_don_id, san_pham_id=item.get("san_pham_id"), ctsp_id=item.get("ctsp_id"), so_luong_ban=item.get("so_luong_ban"), don_vi_tinh=item.get("don_vi_tinh"), gia_ban=item.get("gia_ban"), gia_nhap=item.get("gia_nhap"), thanh_tien=thanh_tien, chiet_khau=item.get("chiet_khau"), la_qua_tang=item.get("la_qua_tang")
            )
        ...
        print("total_amount before:", total_amount)
        ...
        total_amount = round(total_amount * (1 + vat/100), 2)
...
def add_ct_hoa_don_xuat(ds_sku, hoa_don_id, san_pham_id, ctsp_id, so_luong_ban, don_vi_tinh, gia_ban, gia_nhap, thanh_tien, chiet_khau, la_qua_tang):
    ...
    for item_sku in ds_sku:
        ...
        ct_xuat_kho           = ChiTietXuatKho(hoa_don_id=hoa_don_id, san_pham_id=san_pham_id, ctsp_id=ctsp_id, sku=sku,so_luong_ban=so_luong_ban_sku, don_vi_tinh=don_vi_tinh, gia_ban=gia_ban, gia_nhap=gia_nhap, chiet_khau=chiet_khau, thanh_tien=thanh_tien_sku, loi_nhuan=loi_nhuan_sku, la_qua_tang=la_qua_tang)
...
def tra_hang(hoa_don_id, ds_san_pham_tra):
    try:
        ...
        for san_pham_tra in ds_san_pham_tra:
            id = san_pham_tra.get("id")
            so_luong_tra = san_pham_tra.get("so_luong")
            ...
            ton_kho.so_luong_ton = ton_kho.so_luong_ton + so_luong_tra
        db.session.commit()
        return get_error_response(ERROR_CODES.SUCCESS)
    except Exception as e:
        print(e)
